...
class ReceiverInterface:
   ...
   def set_demo_gui(self):
      ...
      #import teste as image_control
 
      #self.wx_app = wx.App()
      #self.demo_frame = LPLOT.Frame(title='teste')
      #self.demo_frame.EnableCloseButton(True)
      #self.demo_frame.Show(True)
      #self.demo_frame.style  = 'graph'
...
def compute_TR_data(rec):
   ...
   ** Please add each data_choice to the -help.  Search for motion_norm to
   ...
   if rec.data_choice == 'motion':
      if rti.nread > 0:
         motion = [rti.motion[ind][rti.nread-1] for ind in range(6)]
         #rec.demo_frame.plot_data(motion)
   ...
   #                     --> sqrt(sum of squared motion params)
      ...
      if rti.nread > 0:
         motion = [rti.motion[ind][rti.nread-1] for ind in range(6)]
         print 'XX - PrevMotion: ', rec.Previousmotion
         for i in range(6):   
             rec.Currentmotion[i] = motion[i]-rec.Previousmotion[i]
	 rec.Previousmotion = motion  
         #derivative = UTIL.derivative(motion,in_place=0, direct=0)
         #derivative2 = UTIL.derivative(motion,in_place=0, direct=1)
         euclideanMotionDerivative = UTIL.euclidean_norm(rec.Currentmotion)
         ...
         print 'XX - CurrMotion: ', rec.Currentmotion
         ...
         return 0, [UTIL.euclidean_norm(rec.Currentmotion)]
      else: return -1, []
